...
/**
 * Custom FAQ Section Wrapper for gosgconsulting
 * Renders FAQ without arrow icons in the title
 */
const FlowbiteFAQSectionWrapper: React.FC<{ items?: any[] }> = ({ items = [] }) => {
  ...
  const schemaItems = componentSchema.items || [];

  const getHeading = (key: string) => {
    const item = schemaItems.find(
  ...
  const faqItems = useMemo<FAQItem[]>(() => {
    ...
    const numberedArrays = schemaItems
  ...
  return (
    <section
      className={`relative overflow-hidden py-20 px-4 bg-[color:var(--bg-primary)]`}
    >
