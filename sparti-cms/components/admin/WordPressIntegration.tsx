...
/**
 * WordPress Integration component that displays integration status
 */
export const WordPressIntegration: React.FC<WordPressIntegrationProps> = ({ tenant }) => {
  ...
  useEffect(() => {
    ...
    const checkIntegrationStatus = async () => {
      try {
        ...
        const response = await fetch(`/api/tenants/${tenant.id}/integrations/wordpress`, {
          headers: {
        ...
        console.error('[testing] Error checking WordPress integration status:', error);
        setIsActive(false);
      } finally {
        setIsLoading(false);
      }
...
/**
 * WordPress Integration list item for modals
 */
export const WordPressIntegrationListItem: React.FC<{ tenant?: Tenant }> = ({ tenant }) => {
  if (!tenant) {
    return <li>• WordPress (No tenant selected)</li>;
  }
  
  return (
    <li>• WordPress (Tenant: {tenant.name})</li>
  );
};
