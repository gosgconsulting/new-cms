...
async function getAstroHandler() {
  ...
  if (!existsSync(ASTRO_SERVER_ENTRY)) {
    try {
      ...
      console.error('[masterastrowind-astro] Build error:', err);
      return null;
    }
  }
...
async function resolveTenantId(req) {
  ...
  // Fallback: pick the first tenant using theme_id = 'masterastrowind'
  const result = await query(
    `SELECT id FROM tenants WHERE theme_id = 'masterastrowind' ORDER BY created_at DESC LIMIT 1`,
    []
  );
...
router.use(async (req, res, next) => {
  try {
      ...
      return res
        ...
        .send(
          `<html><body style="font-family: system-ui; padding: 24px;">
            <h1>Master Astrowind theme is not built yet</h1>
    ...
    console.error('[masterastrowind-astro] SSR error:', err);
    return next(err);
  }
});
