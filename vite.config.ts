...
export default defineConfig(({ mode }) => {
  ...
  const plugins = [
    // Copy src/assets and public to dist so URLs work in production
    // Copy src/assets and public to dist so URLs work in production
    {
      ...viteStaticCopy({
        targets: [
          { src: 'src/assets', dest: 'src', structured: true },
          {
            src: 'public/**',
            dest: '.',
            structured: true,
            rename: (_fileName, _ext, fullPath) => fullPath.replace(/^public\//, ''),
          },
        ],
      }),
      apply: 'build',
