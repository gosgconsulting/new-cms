-- 1) Create tracked keywords table for SEO
...
-- Unique per user/brand/keyword
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'uniq_tracked_keyword_per_brand_user'
  ) THEN
...
-- RLS policies: allow user or workspace members of brand
...
-- 2) Ensure blog_posts link to campaigns
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3) Backfill: create campaign records for historical blog posts not linked yet
...
DECLARE
  ...
  v_campaign_id uuid;
BEGIN
  -- Group posts by user, brand and group_date
  FOR rec IN
    SELECT
      bp.user_id,
      bp.brand_id,
      COALESCE(
        (bp.campaign_creation_date)::date,
        (bp.published_date)::date,
        (bp.scheduled_date)::date,
        (bp.created_at)::date
      ) AS group_date
  ...
  LOOP
    -- Reuse existing campaign if same day exists for this user/brand
    ...
    -- Attach posts in that group to the campaign
...
-- 4) RPC to fetch unified campaigns for a brand (used by UI)
...
BEGIN
  ...
  SELECT 
    ...
    c.updated_at,
    COALESCE(
      (SELECT ARRAY(SELECT DISTINCT unnest( (SELECT COALESCE( (SELECT array_agg(DISTINCT k) FROM unnest(ARRAY(SELECT jsonb_array_elements_text(c.keywords))) k), '{}'::text[] ) ))) ),
      '{}'::text[]
    ) AS keywords,
...
-- 5) RPC to create a share for a grouped campaign date
...
DECLARE
  ...
  v_share_id uuid;
BEGIN
  -- Basic permission check: brand is owned by user or in same workspace
  IF NOT (
...
-- 6) Execute the backfill now (safe to run multiple times)
