...
-- Create index for efficient querying (without WHERE clause in index)
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'places_autocomplete_cache' AND policyname = 'Allow public read access to autocomplete cache'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'places_autocomplete_cache' AND policyname = 'Allow public insert to autocomplete cache'
  ) THEN
