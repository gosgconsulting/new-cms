...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'business_lead_deduplication' AND policyname = 'Users can view their campaign deduplication data'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'business_lead_deduplication' AND policyname = 'Service role can manage deduplication data'
  ) THEN
