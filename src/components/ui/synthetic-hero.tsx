...
const ShaderPlane = ({ vertexShader, fragmentShader, uniforms }: ShaderPlaneProps) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      (material.uniforms as any).u_time.value = state.clock.elapsedTime * 0.5;
      (material.uniforms as any).u_resolution.value.set(size.width, size.height, 1.0);
    }
  });
...
const SyntheticHero: React.FC<HeroProps> = ({
