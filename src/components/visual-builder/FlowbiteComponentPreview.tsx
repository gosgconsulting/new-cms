...
const findImageItem = (items: any[]) => {
  return items.find((i) => i?.type === "image" && typeof i?.src === "string") || null;
};
...
function SectionHero({ items }: { items: SchemaItem[] }) {
  ...
  const logo = items.find((i) => i.key?.toLowerCase() === "logo" && i.type === "image") as any;
  ...
  return (
    <section className="relative w-full min-h-[40vh] overflow-hidden rounded-lg">
      <img
        src={currentImg.src}
      ...
      {hasSlides && slides.length > 1 ? (
        <div className="absolute inset-x-0 bottom-4 flex items-center justify-center gap-2 z-10">
          <button onClick={prev} className="rounded-lg bg-white/80 px-3 py-1 text-sm hover:bg-white">
            Prev
          </button>
          ...
          <button onClick={next} className="rounded-lg bg-white/80 px-3 py-1 text-sm hover:bg-white">
            Next
          </button>
        </div>
      ) : null}
...
function SectionServices({ items }: { items: SchemaItem[] }) {
  ...
  const cards = services
    ...
    .map((colItems: any[]) => {
      ...
      const btn = colItems.find((i) => i?.type === "button" && typeof i?.content === "string");
      return { img, btn };
    })
    .filter((c) => c.img?.src);
  ...
  return (
    <FlowbiteSection title={title || undefined} subtitle={subtitle || undefined}>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((card, idx) => (
          <div key={idx} className="w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            ...
            <div className="space-y-1 p-4">
              <div className="flex items-center gap-2">
                {card.img?.title ? <h3 className="text-base font-semibold">{card.img.title}</h3> : null}
                {card.img?.price ? (
              ...
              {card.img?.alt ? <p className="line-clamp-3 text-xs text-gray-500">{card.img.alt}</p> : null}
              {card.btn?.content ? (
                <a href={card.btn.link || "#"} className="mt-3 inline-block rounded-lg bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700">
                  {card.btn.content}
                </a>
              ) : null}
            </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {features.map((f: any, idx: number) => (
          <div key={idx} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            ...
            <div className="space-y-1 p-4">
              {f?.title ? <h3 className="text-base font-semibold">{f.title}</h3> : null}
              {f?.description ? <p className="text-sm text-gray-600">{f.description}</p> : null}
            </div>
          </div>
        ))}
      </div>
...
function SectionIngredients({ items }: { items: SchemaItem[] }) {
  const title = getText(items, "title");
  const subtitle = getText(items, "subtitle");
  const ingredients = getArray(items, "ingredients").filter((ing: any) => !!ing?.src || !!ing?.name || !!ing?.benefit);
      ...
      <div className="mt-6 grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
        {ingredients.map((ing: any, idx: number) => (
          <div key={idx} className="rounded-lg border bg-white p-3">
            ...
            {ing?.name ? <h4 className="mt-2 text-sm font-semibold">{ing.name}</h4> : null}
            {ing?.benefit ? <p className="text-xs text-gray-600">{ing.benefit}</p> : null}
          </div>
        ))}
      </div>
    </FlowbiteSection>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {team.map((m: any, idx: number) => (
          <div key={idx} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            ...
            <div className="space-y-1 p-4">
              {m?.name ? <h3 className="text-base font-semibold">{m.name}</h3> : null}
              {m?.role ? <p className="text-xs text-gray-500">{m.role}</p> : null}
  ...
  return (
    <FlowbiteSection containerClassName="grid items-center gap-6 md:grid-cols-2">
      <div className="space-y-3">
        ...
        {description ? <p className="text-sm md:text-base text-gray-600">{description}</p> : null}
      </div>
      {imageItem?.src ? (
...
const FlowbiteComponentPreview: React.FC<Props> = ({ schema }) => {
...
export default FlowbiteComponentPreview;