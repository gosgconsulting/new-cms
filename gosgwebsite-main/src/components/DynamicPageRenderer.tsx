...
import { componentRegistry, ComponentType } from '@/lib/componentRegistry';
  ...
  return (
    <>
      {schema.components.map((component: ComponentSchema, index: number) => {
        ...
        // SPECIAL CASE: Map ServiceCaseStudies schema items to expected props (title, subtitle, caseStudies)
        if (component.type === 'ServiceCaseStudies') {
          const titleItem = component.items?.find((i: any) => i.key === 'title');
          const subtitleItem = component.items?.find((i: any) => i.key === 'subtitle');
          const caseStudiesItem = component.items?.find((i: any) => i.key === 'caseStudies');

          const mappedCaseStudies = (caseStudiesItem?.items || []).map((cs: any) => {
            const inner = cs.items || [];
            const t = inner.find((i: any) => i.key === 'title' || (i.type === 'heading' && i.level === 3));
            const d = inner.find((i: any) => i.key === 'description' || i.type === 'text');
            const r = inner.find((i: any) => i.key === 'result' || (i.type === 'text' && i.key?.includes('result')));
            const img = inner.find((i: any) => i.type === 'image');
            const linkItem = inner.find((i: any) => i.key === 'link' && i.type === 'text');
            ...
            return {
              ...
              result: r?.content || '',
              image: img?.src,
              link: linkItem?.content || '/contact',
            };
          });

          componentProps = {
            ...componentProps,
            title: titleItem?.content || 'Case Studies',
