      ...
      {testimonials.map((testimonial, index) => {
        ...
        return (
          <div
            ...
            key={testimonial.id}
            className="testimonial-card glass-effect backdrop-blur-xl absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-card/80 border border-border rounded-2xl shadow-2xl transition-all duration-300 cursor-grab active:cursor-grabbing"
      ...
      <div className="pagination flex gap-2 justify-center absolute bottom-0 left-0 right-0">
        {testimonials.map((_, index) => (
          <button 
            ...
            className={`w-2 h-2 rounded-full transition-colors ${activeIndex === index ? 'bg-primary' : 'bg-primary/20'}`} 
          />
        ))}
      </div>
    </section>
